...
import { Card, Kpi, Loading, ErrorState, Tooltip, PageHeader } from '../components/ui.jsx';
...
export default function Forecast() {
  ...
  if (fc.loading) return <Loading chart />;
  const d = fc.data;
  ...
  return (
    <div className="stack">
      <PageHeader eyebrow="Predictive" title="Forecast" subtitle={<>Linear-trend projection over the last 60 days with an 80% confidence band</>}>
      ...
      <div className="grid grid-4">
        <Kpi label="Month to date" raw={d.monthToDate} format={fmtMoney} />
        <Kpi label="Projected month end" raw={d.projectedMonthEnd} format={fmtMoney} hint="MTD + projected remaining days" />
        <Kpi label="Daily run rate" raw={d.dailyRunRate} format={fmtMoney} hint="7-day average" />
      ...
      <Card title="Actual vs forecast" subtitle={`Next ${horizon} days projected total: ${fmtMoney(d.projectedNext30Days)}`}>
        <ResponsiveContainer width="100%" height={380}>
          <ComposedChart data={data} margin={{ top: 5, right: 5, left: 0, bottom: 0 }}>
            ...
            <Area type="monotone" dataKey="band" name="80% band" stroke="none" fill="var(--accent)" fillOpacity={0.15} animationDuration={1200} legendType="none" tooltipType="none" />
            <Line type="monotone" dataKey="Actual" stroke="var(--primary)" strokeWidth={2.2} dot={false} animationDuration={1200} style={{ filter: "drop-shadow(0 0 6px rgba(34,211,238,.5))" }} />
            <Line type="monotone" dataKey="Forecast" stroke="var(--accent)" strokeWidth={2.2} strokeDasharray="6 4" dot={false} animationDuration={1600} />
